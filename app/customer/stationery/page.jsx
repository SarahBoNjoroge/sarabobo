...
export default function StationeryPage() {
  ...
  const addToCart = (item) => {
    ...
    if (existing) {
      ...
      cart.push({
        ...
        quantity,
        image: item.image || null, // ✅ FIXED: save image
      });
    }
  ...
  return (
    ...
    >
      <div className="absolute top-4 right-4 z-20"><CartIcon /></div>

      <div className="flex justify-center gap-6 mb-8">
        <button onClick={() => router.push('/customer')} className="text-purple-800 font-semibold hover:underline">🏠 Home</button>
        <button onClick={() => router.push('/customer/dashboard')} className="text-purple-800 font-semibold hover:underline">📚 Books</button>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
        {items.map(item => (
          <div key={item.id} className="bg-white p-4 rounded shadow">
            ...
            />
            <h2 className="font-semibold text-lg cursor-pointer text-blue-600 hover:underline" onClick={() => router.push(`/customer/stationery/${item.id}`)}>
              {item.name}
            </h2>
            <p className="font-bold text-green-600 mt-2">Ksh {parseFloat(item.price).toFixed(2)}</p>
